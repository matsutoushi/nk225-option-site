...
# 日本語ページの meta description。
# これが無いと検索結果のスニペットをGoogleが本文から適当に切り出すため、
# 順位が取れていてもクリックされない(2026-08-27時点で「ガンマエクスポージャー」は
# 18回表示されてクリック0だった)。各記事で一番具体的な事実を先頭に置く。
GUIDE_DESC = {
    "glossary.html":
        "建玉・SQ・限月・デルタなど、日経225オプションのデータを読むときに出てくる用語を、"
        "どのJPX公表ファイルに載っている数字なのかまで含めてまとめました。",
    "guide-lee-ready.html":
        "約定を買い仕掛けと売り仕掛けに分けるLee-Readyアルゴリズムの考え方と、"
        "ディーラーのガンマ推定に必要なデータ(約定・気配のティック)。"
        "JPXの無料公開データだけでは実行できない理由と、当サイトが一律の仮定を使っている理由を説明します。",
    "guide-gex.html":
        "日経225のガンマエクスポージャー(推定)を毎営業日更新しています。合計値と現値の上下の内訳つき。"
        "ガンマエクスポージャーは、証券会社のヘッジ売買が値動きを増幅するか抑制するかの推定値です。"
        "JPXが公表する行使価格別のボラティリティから計算する方法と、"
        "「ディーラーの持ち高は公表されていない」という前提の限界まで説明します。",
    "guide-cot.html":
        "米CFTCが週次で公表するCOTレポートの読み方。投機筋の建玉が片側に偏りきった状態からの反転が"
        "使いどころで、火曜集計・金曜公表のため常に3営業日遅れる点も含めて解説します。",
    "guide-sq.html":
        "SQ算出日は毎月第2金曜(休業日なら繰り上げ)、SQ値が決まるのは225銘柄が寄り付いた当日の午前中で、"
        "JPXの公式掲載は15:45以降です。取引最終日は前営業日。次回SQの日程と過去24か月のSQ値、"
        "日経平均始値との差(平均225円・最大1,522円)を毎営業日更新しています。",
    "guide-teguchi.html":
        "JPXは証券会社名を明示して週次の建玉を公表しています。ただしゼロと比べても意味がなく、"
        "HSBC証券は52週すべて売り越しでした。その会社自身の平常値と比べる読み方を解説します。",
    "guide-oi.html":
        "日経225オプションの建玉分布(市場建玉分布)を毎営業日更新しています。現値近辺で建玉が最も厚い行使価格と、"
        "行使価格ごとの残高・前日比を掲載。"
    ...
    "guide-pcr.html":
        ...
        "分子だけでなく分母も見る必要がある理由を説明します。",
    "guide-start.html":
        "日経225オプションの口座開設から取引開始までの一般的な流れと、"
        "主要ネット証券のミニオプション取扱い・手数料の比較。"
        "1万円以下から試せる条件がどこにあるかを公表値で整理しました。",
    "guide-brokers.html":
...
# ---------------------------------------------------------------------------
# SQページ(guide-sq.html)は数字を毎日入れ替える
#
# Search Console(2026-09-05)では sq 何時 / sq 時間 / sq 値 / sq いつ / sq日 /
# sq 気配 など23語が表示されていたが、掲載順位は30〜95位でクリックはほぼゼロだった。
# 検索意図は概念の説明ではなく「いつ・何時・いくらだったか」なので、
# 次回SQのカレンダーと過去のSQ値の実績を、data/ の生成物から組み立てる。
# ---------------------------------------------------------------------------

def _sq_data():
    """(次回SQ以降のカレンダー, SQ値の履歴DataFrame)。取れなければ (None, None)。"""
    try:
        ...
        cal = _sq.upcoming(today, 12)
        path = _os.path.join(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))),
                             "data", "sq_history.csv")
        hist = _pd.read_csv(path) if _os.path.exists(path) else None
        return cal, hist
    except Exception:
        return None, None


def _sq_calendar_html(cal):
    if not cal:
        return ""
    nxt = cal[0]
    kind = "メジャーSQ" if nxt["major"] else "マイナーSQ"
    when = "本日" if nxt["days"] == 0 else ("明日" if nxt["days"] == 1 else f'あと{nxt["days"]}日')
    rows = "".join(
        '<tr><td>{m}</td><td><b>{sq}</b>({w})</td><td>{last}</td><td>{k}</td></tr>'.format(
            m=r["month"].replace("-", "年") + "月限",
            sq=r["sq"].strftime("%Y/%m/%d"),
            w="月火水木金土日"[r["sq"].weekday()],
            last=r["last"].strftime("%m/%d"),
            k="メジャー" if r["major"] else "マイナー")
        for r in cal)
    return (
        '<div style="margin:18px 0; padding:14px 16px; border:1px solid #dfe3e9;'
        ' border-radius:10px; background:#ffffff;">'
        '<p style="margin:0 0 6px; font-size:1.05em;"><b>次回SQ算出日: {sq}({w})</b> — {when}</p>'
        '<p style="margin:0; font-size:0.95em; color:#4b5563;">'
        '{kind}。取引最終日はその前営業日の <b>{last}</b> です。</p></div>'
        '<div class="tbl-wrap"><table>'
        '<thead><tr><th>限月</th><th>SQ算出日</th><th>取引最終日</th><th>区分</th></tr></thead>'
        '<tbody>{rows}</tbody></table></div>'
    ).format(sq=nxt["sq"].strftime("%Y年%m月%d日"), w="月火水木金土日"[nxt["sq"].weekday()],
             when=when, kind=kind, last=nxt["last"].strftime("%m月%d日"), rows=rows)


def _sq_history_html(hist):
    """過去のSQ値と、同じ日の日経平均始値との差。"""
    if hist is None or not len(hist):
        return "", ""
    d = hist.dropna(subset=["diff"])
    if not len(d):
        return "", ""
    gap = d["diff"].abs()
    over = int((d["diff"] > 0).sum())
    big = int((gap > 500).sum())
    stats = (
        '<p>当サイトでSQ値と同じ日の日経平均始値を突き合わせたところ、'
        '<b>{n}か月分</b>({first}〜{last})で次のようになりました。</p>'
        '<ul>'
        '<li>差の<b>平均は{mean:,.0f}円</b>、中央値は{med:,.0f}円</li>'
        '<li>もっとも開いた月は<b>{mx:,.0f}円</b>({mxm})</li>'
        '<li><b>500円以上ずれた月が{big}回</b>({bigp:.0f}%)ありました</li>'
        '<li>SQ値が始値を上回ったのが{over}か月、下回ったのが{under}か月</li>'
        '</ul>'
        '<p>メジャーSQでの平均は{maj:,.0f}円、マイナーSQでは{mnr:,.0f}円。'
        '<b>メジャーSQだからずれが大きい、というわけではありません。</b></p>'
    ).format(
        n=len(d), first=d["month"].iloc[0], last=d["month"].iloc[-1],
        mean=gap.mean(), med=gap.median(), mx=gap.max(),
        mxm=d.loc[gap.idxmax(), "month"], big=big, bigp=100.0 * big / len(d),
        over=over, under=len(d) - over,
        maj=d[d["major"]]["diff"].abs().mean(), mnr=d[~d["major"]]["diff"].abs().mean())

    tail = hist.tail(24).iloc[::-1]
    rows = []
    for _, r in tail.iterrows():
        op = "—" if r["nikkei_open"] != r["nikkei_open"] else "{:,.2f}".format(r["nikkei_open"])
        df = "—" if r["diff"] != r["diff"] else "{:+,.0f}".format(r["diff"])
        rows.append(
            "<tr><td>{m}</td><td>{d}</td><td><b>{v:,.2f}</b></td><td>{o}</td><td>{f}</td></tr>".format(
                m=r["month"].replace("-", "年") + "月限" + ("(メジャー)" if r["major"] else ""),
                d=r["sq_date"], v=r["sq_value"], o=op, f=df))
    table = ('<div class="tbl-wrap"><table>'
             '<thead><tr><th>限月</th><th>算出日</th><th>SQ値</th>'
             '<th>同日の日経平均始値</th><th>差</th></tr></thead>'
             '<tbody>' + "".join(rows) + '</tbody></table></div>')
    return stats, table


_SQ_CAL, _SQ_HIST = _sq_data()
_SQ_CALENDAR = _sq_calendar_html(_SQ_CAL)
_SQ_STATS, _SQ_TABLE = _sq_history_html(_SQ_HIST)
...
<p>当サイトに出てくる用語を、<b>その数字がどの公表ファイルから来ているか</b>まで含めて並べます。
出どころが分かると、自分で確かめたいときに辿れます。
ファイル名と公表時刻の一覧は<a href="guide-jpx-data.html">公式データはどこにあるか</a>にまとめています。</p>

<h2>オプションの基本</h2>
<ul>
<li><b>建玉(たてぎょく / Open Interest)</b> — まだ決済されずに残っている契約の総量。
出来高が「その日に売買された量」なのに対し、建玉は「積み上がっている残高」です。
同じ1枚の売買でも、双方が新規なら建玉は増え、双方が決済なら減り、片方ずつなら変わりません。
JPXの <code>open_interest.xlsx</code> が出どころで、公表は20時頃。
→ <a href="guide-oi.html">建玉分布の見方</a></li>

<li><b>行使価格(権利行使価格 / ストライク)</b> — その値段で買う(売る)権利、と決められた価格。
日経225オプションは直近3限月が125円刻み、それより先が250円刻みです。
実際には申請による追加設定もあり、9月限では298本が立っていました。</li>

<li><b>限月(げんげつ)</b> — その契約が満期を迎える月。「26年9月限」のように呼びます。
期近(いちばん手前)がいつも最大とは限らず、メジャーSQのある四半期の限月に
建玉が集まることがあります。</li>

<li><b>壁</b> — 特定の行使価格に建玉が集中した状態の通称。
売り手のヘッジ売買がその周辺で増えるため、値動きの性質が変わりやすい水準とされます。
ただし<b>建玉が最大の行使価格がそのまま壁になるわけではありません</b>。
現値から何割も離れた行使価格に最大建玉があることは珍しくないためです。
→ <a href="guide-oi.html">「最大建玉=壁」ではない</a></li>

<li><b>ITM / ATM / OTM</b> — 権利行使すると得になる状態がイン・ザ・マネー(ITM)、
現値とほぼ同じがアット・ザ・マネー(ATM)、権利行使しても損なのがアウト・オブ・ザ・マネー(OTM)。
OTMのまま満期を迎えたオプションは価値ゼロで消滅します。</li>

<li><b>プレミアム</b> — オプションの価格。日経225オプションは
「オプション価格 × 1,000円」が1枚の金額で、ミニオプションは × 100円です。
→ <a href="guide-start.html">取引の始め方</a></li>
</ul>

<h2>需給を読むための指標</h2>
<ul>
<li><b>Put/Callレシオ(PCR)</b> — プット出来高 ÷ コール出来高。
<b>1.0が中立ではありません。</b>日経225オプションは機関投資家の下落ヘッジが恒常的に入るため、
常時1.0を超えているのが普通です。水準より変化を見ます。
また分母(コール)が減るだけでも上がるため、<b>両方を見ないと逆に読みます</b>。
JPXの当日取引高ファイルが出どころ。→ <a href="guide-pcr.html">解説</a></li>

<li><b>IV(インプライド・ボラティリティ)</b> — オプションの値段から逆算した、
市場が織り込んでいる将来の変動率。JPXは<b>行使価格ごとのIV</b>を清算値段CSV
(<code>rbYYYYMMDD.csv</code>、18時頃)で公表しています。
深いITMは1.0%固定のダミー値、裾では同じ値が使い回されるため、
当サイトはOTM側だけを採用して表示しています。</li>

<li><b>スマイル / スキュー</b> — 行使価格ごとにIVを並べたときの形。
下方向のIVが上方向より高いのが株式市場の常態で、
下落に備える需要が強いほど左側が持ち上がります。
日経VIという1つの数字では、この偏りは見えません。</li>

<li><b>ガンマ・エクスポージャー(GEX)</b> — オプションを売った側のヘッジ売買が、
相場を抑える向きに働くか増幅する向きに働くかの推定値。
<b>公表値ではなく当サイトの計算です</b>。建玉と行使価格別IVから算出し、
「ディーラーはコールを買い持ち・プットを売り持ち」という一般的な前提を置いています。
→ <a href="guide-gex.html">解説</a></li>

<li><b>デルタヘッジ</b> — オプションの売り手が、値動きのリスクを打ち消すために
先物などを売買すること。この売買自体が相場を動かす一因になります。
満期が近いほど反応が鋭くなります。</li>

<li><b>レバレッジETFのリバランス</b> — TQQQ・SOXL等が一定倍率を保つため引けに行う売買。
上昇日は買い・下落日は売りで、値動きを増幅する方向に働きます。
<b>オプションのガンマとは別のメカニズムなので、単純に足し合わせることはできません</b>。
→ <a href="us.html">米国市場データ</a></li>
</ul>

<h2>満期まわり</h2>
<ul>
<li><b>SQ(特別清算指数)</b> — 満期を迎えた限月を清算するための価格。
毎月第2金曜が算出日で、休業日なら前営業日に繰り上がります。
<b>225銘柄それぞれの寄り付き値から算出</b>されるため、日経平均の始値とは別物です。
44か月の実測では平均225円、最大1,522円ずれていました。→ <a href="guide-sq.html">解説</a></li>

<li><b>メジャーSQ / マイナーSQ</b> — 3・6・9・12月は先物も同時に満期を迎えるため
メジャーSQと呼ばれ、清算される金額が大きくなります。それ以外の月はオプションのみ。</li>

<li><b>取引最終日</b> — 売買できる最後の日で、<b>SQ算出日ではなくその前営業日</b>です。
第2金曜がSQなら木曜が最終日になります。</li>

<li><b>ウィークリーオプション</b> — 日経225ミニオプションには週次の満期があり、
2025年5月からは水曜満期の限月も加わりました。
月次SQでない週にも満期に伴う需給が発生しています。</li>
</ul>

<h2>ポジションデータ</h2>
<ul>
<li><b>手口(取引参加者別取引高)</b> — どの証券会社を通じて何枚売買されたかの集計。
JPXが平日17:45頃に公表します。<b>買いか売りかは含まれません</b>。
→ <a href="guide-teguchi.html">手口の見方</a></li>

<li><b>取引参加者別建玉</b> — 証券会社ごとのネット建玉を<b>実名で</b>公表する週次データ
(<code>indexfut_oi_by_tp.xlsx</code>)。手口と違い<b>方向が分かります</b>。
米国のCOTが商業筋・非商業筋という3分類までなのに対し、JPXは30社前後を実名で出します。
ただし上位は清算・執行の受け皿で、自社の相場観ではありません。
→ <a href="guide-brokers.html">手口に出てくる証券会社</a></li>

<li><b>COTレポート</b> — CFTC(米)が週次公表する投資家区分別の先物建玉。
火曜集計・金曜公表で常に3営業日遅れます。→ <a href="guide-cot.html">解説</a></li>

<li><b>投資部門別売買状況</b> — JPXが週次公表する、海外投資家・個人などの現物売買代金。</li>

<li><b>ラージ換算</b> — ラージ・ミニ・マイクロは取引単位が10倍ずつ違うため、
...
<h2>ボラティリティ・マクロ</h2>
<ul>
<li><b>日経VI</b> — 日経平均のオプション価格から算出される期待変動率の指数。
20超で警戒領域、30超は荒れ相場とされますが、年によって水準が大きく違い、
固定の線より直近の中での位置で見るほうが実態に合います。日本経済新聞社が公表しています。
→ <a href="nikkei-vi.html">日経VIの今日の値と実測</a></li>

<li><b>VIX</b> — S&amp;P500版の同種の指数。</li>

<li><b>ブレークイーブン(BEI)</b> — 物価連動債と普通国債の利回り差で、
債券市場が織り込んでいる期待インフレ率。実績の物価ではなく<b>市場の予想</b>です。
→ <a href="risk.html">マクロリスクモニター</a></li>

<li><b>Sahmルール</b> — 失業率の3か月平均が直近1年の最低から0.5ポイント上がったら
景気後退入り、という経験則。当たってきた一方で<b>後追いの指標</b>です。
→ <a href="risk.html">マクロリスクモニター</a></li>

<li><b>イールドカーブの逆転</b> — 長期金利が短期金利を下回る状態。
...
<h2>あわせて読む</h2>
<ul>
<li><a href="guide-jpx-data.html">公式データはどこにあるか</a> — ファイル名・公表時刻の一覧</li>
<li><a href="guide-oi.html">建玉分布とは</a> — 「壁」の読み方と誤読</li>
<li><a href="guide-sq.html">SQとは</a> — 次回の日程と過去のSQ値</li>
<li><a href="about.html">運営者情報</a> — 出典と、推計を含む項目</li>
</ul>
"""),
    "guide-lee-ready.html": ("約定を買い・売りに分ける — Lee-Readyとガンマ推定", """
<h1>約定を「買い」と「売り」に分ける — Lee-Readyアルゴリズムとガンマ推定</h1>
<p>ガンマエクスポージャーの計算で、いちばん弱いのは値動きの式ではありません。
<b>「証券会社(ディーラー)がどちら向きに持っているか」が公表されていない</b>ことです。
ここを埋める方法として使われるのが、約定を買い仕掛けと売り仕掛けに分ける
<b>Lee-Readyアルゴリズム</b>です。このページでは、その考え方と、
当サイトがなぜそれを使っていないのかを説明します。</p>

<h2>建玉には「向き」が書かれていない</h2>
<p>JPXが公表する建玉は、行使価格ごとに<b>何枚残っているか</b>だけです。
1枚の建玉は必ず買い手と売り手がいて成り立ちますが、
そのどちらが証券会社かは書かれていません。</p>
<p>ところがガンマの向きは、まさにそこで決まります。
ディーラーがコールを<b>買って</b>持っていれば、値動きを抑える売買をします。
<b>売って</b>持っていれば、値動きを増幅する売買になります。
同じ建玉でも、向きが逆なら結論が正反対になります。
→ <a href="guide-gex.html">ガンマエクスポージャーとは</a></p>

<h2>Lee-Readyの考え方</h2>
<p>1991年にチャールズ・リーとマーク・レディが示した、
<b>約定を「買い手が仕掛けたか」「売り手が仕掛けたか」に分ける</b>手順です。
株式市場の研究で広く使われ、今もオプションの需給推定の土台になっています。</p>
<ol>
<li><b>気配の中値と比べる。</b>売り気配と買い気配の真ん中より高い値段で約定していれば、
買い手が売り気配に当てにいった=<b>買い仕掛け</b>。真ん中より低ければ<b>売り仕掛け</b></li>
<li><b>ちょうど中値なら、直前の約定と比べる(ティックテスト)。</b>
前の約定より高ければ買い仕掛け、低ければ売り仕掛け</li>
</ol>
<div class="tbl-wrap"><table>
<thead><tr><th>例</th><th>買い気配</th><th>売り気配</th><th>約定値段</th><th>判定</th></tr></thead>
<tbody>
<tr><td>1</td><td>300</td><td>310</td><td>310</td><td>買い仕掛け(中値305より上)</td></tr>
<tr><td>2</td><td>300</td><td>310</td><td>300</td><td>売り仕掛け(中値305より下)</td></tr>
<tr><td>3</td><td>300</td><td>310</td><td>305</td><td>直前の約定が300なら買い仕掛け、310なら売り仕掛け</td></tr>
</tbody>
</table></div>
<p>仕掛けた側(成行で相手の気配を取りにいった側)は、多くの場合<b>顧客</b>です。
気配を出して待っていた側は、マーケットメイクをしている証券会社であることが多い。
そこで<b>「仕掛けた側の反対がディーラー」</b>と考えると、
行使価格ごとにディーラーがどちら向きに持っているかを推定できます。</p>

<h2>これをやるのに必要なデータ</h2>
<p>ここが実務上の壁です。Lee-Readyには<b>約定1件ごとの値段と、その瞬間の売り気配・買い気配</b>が要ります。
日次の建玉や清算値段では計算できません。</p>
<div class="tbl-wrap"><table>
<thead><tr><th>データ</th><th>入手</th><th>Lee-Readyに使えるか</th></tr></thead>
<tbody>
<tr><td>建玉残高(行使価格別・日次)</td><td>JPXが無料公開</td><td>量は分かるが<b>向きは分からない</b></td></tr>
<tr><td>清算値段・行使価格別IV(日次)</td><td>JPXが無料公開</td><td>ガンマの計算には使えるが、向きは分からない</td></tr>
<tr><td>取引参加者別の取引高(日次)</td><td>JPXが無料公開</td><td>会社名は分かるが<b>売りか買いかは含まれない</b></td></tr>
<tr><td>約定・気配のティック</td><td>取引所の有償配信、または情報端末</td><td><b>これが必要</b></td></tr>
</tbody>
</table></div>
<p>つまり、<b>無料で公開されているデータだけではLee-Readyは実行できません</b>。
公開ファイルの一覧は<a href="guide-jpx-data.html">公式データはどこにあるか</a>にまとめています。</p>

<h2>当サイトがしていること、していないこと</h2>
<p>当サイトの<a href="guide-gex.html">ガンマエクスポージャー</a>は、
<b>「ディーラーはコールを買い持ち・プットを売り持ち」という一律の仮定</b>で計算しています。
広く使われている前提ですが、<b>Lee-Readyのような実測に基づくものではありません</b>。
トップページと解説ページで「推定値」と書いているのはこのためです。</p>
<p>この仮定が外れる場面もあります。たとえば個人や機関投資家が上のコールを買い越していれば、
ディーラーはそのコールを<b>売って</b>いることになり、仮定とは逆になります。
そのときガンマの符号は反転します。<b>符号が反対になれば、相場を抑える/増幅するの結論も反対</b>です。
ここは、公開データだけでは検証できない部分として残ります。</p>

<h2>もし向きを測るなら: 量は建玉、向きは約定</h2>
<p>符号付きの出来高をそのまま積み上げるやり方には、弱点があります。
<b>立会外(J-NET)の取引はティックに出てこない</b>ため、
その分で作られた建玉が抜け落ちます。</p>
<p>そこで役割を分けるほうが筋がよくなります。</p>
<ul>
<li><b>量</b>は、JPXの建玉の増減から取る(取りこぼしがない)</li>
<li><b>向き</b>は、その日の約定の偏り(Lee-Ready)から取る</li>
<li>その日の出来高が建玉の増減に比べて極端に少ない日は、向きの判断材料が足りないので除く</li>
</ul>
<p>この形なら、建玉の裏付けを保ったまま向きを推定できます。
ただし<b>「仕掛けた側=顧客」という仮定は残ります</b>。
ディーラーが自分から仕掛けることも、機関投資家が指値で待つこともあるためです。</p>

<h2>建玉と出来高の関係(JPXの公表データ)</h2>
{latest_flow}
<p>建玉の増減は出来高の一部でしか起きません。新規どうしの売買なら建玉は増え、
決済どうしなら減り、片方ずつなら変わらないためです。
出来高に対して建玉がどれだけ動いたかを見ると、その日の売買が
<b>新しくポジションを作る動きだったのか、閉じる動きだったのか</b>の目安になります。</p>

<h2>まとめ</h2>
<ul>
<li>建玉は量だけ。向きは公表されていない</li>
<li>Lee-Readyは約定と気配から向きを推定する方法で、オプションの需給推定の土台になっている</li>
<li>実行にはティックデータが要る。<b>無料の公開データだけでは計算できない</b></li>
<li>当サイトのガンマは一律の仮定による推定で、この点は<b>計算を精緻にしても解消しません</b></li>
</ul>
<p>関連: <a href="guide-gex.html">ガンマエクスポージャーとは</a> ・
<a href="strategy-gamma-trading.html">デルタヘッジで何が起きるか</a> ・
<a href="guide-jpx-data.html">公式データはどこにあるか</a> ・
<a href="guide-teguchi.html">手口の見方</a></p>
"""),
    "guide-gex.html": ("日経225のガンマエクスポージャー(毎日推定)｜今日の値と見方", """
<h1>日経225のガンマエクスポージャー — 今日の推定値と、その見方</h1>
{latest_gex}
<p>当サイトのトップページ(日経225)と<a href="us.html">米国市場ページ</a>で毎日更新している
ガンマエクスポージャーの読み方を解説します。
前提となる仮定とその限界まで、できるだけ正直に説明します。</p>

<p>この「ディーラーがどちら向きに持っているか」を、約定データから測る方法もあります
(→ <a href="guide-lee-ready.html">約定を買い・売りに分ける — Lee-Ready</a>)。</p>
...
"""),
    "guide-sq.html": ("SQとは — 次回の日程とSQ値の決まり方", """
<h1>SQとは — 次回はいつ、SQ値は何時に決まるのか</h1>
<p><b>SQ算出日は毎月第2金曜日</b>(休業日なら前営業日に繰り上げ)、
<b>SQ値が決まるのはその日の午前中</b>——日経225の225銘柄すべてが寄り付いた時点です。
取引最終日はSQ算出日ではなく<b>その前営業日</b>なので、木曜までに手仕舞う必要があります。</p>
""" + _SQ_CALENDAR + """
<h2>SQ値は何時に決まるのか</h2>
<p>ここが混乱しやすいところで、<b>「決まる時刻」と「JPXが公表する時刻」は別</b>です。</p>
<div class="tbl-wrap"><table>
<thead><tr><th>いつ</th><th>何が起きるか</th></tr></thead>
<tbody>
<tr><td>前営業日(木曜)</td><td><b>取引最終日</b>。ここまでに決済しなければ、残った建玉は自動的に清算されます</td></tr>
<tr><td>当日 08:00〜09:00</td><td>各銘柄の寄り前気配が出ます。「SQ気配」と呼ばれるのはこれをもとにした推定値で、確定値ではありません</td></tr>
<tr><td>当日 09:00〜</td><td>225銘柄が順に寄り付きます。<b>全部そろった時点でSQ値が確定</b>します。多くの場合9時台ですが、寄り付かない銘柄があると遅れます</td></tr>
<tr><td>当日 15:45以降</td><td>JPXが最終清算数値として公式に掲載します</td></tr>
</tbody>
</table></div>
<p>つまり「SQ値は何時に分かるか」の答えは<b>朝の9時台が目安</b>で、
<b>公式の掲載は夕方</b>ということになります。
朝の時点で流れている数字は各社が独自に集計したもので、JPXの正式公表を待つ必要はありませんが、
最終確定はJPXの掲載をもって、という整理です。</p>
<p style="font-size:0.85em; color:#4b5563;">
公表時刻は
<a href="https://www.jpx.co.jp/markets/derivatives/special-quotation/index.html" rel="nofollow">JPXの最終清算数値ページ</a>に
「原則として15:45以降」と明記されています。</p>

<h2>【実データ】SQ値は日経平均の始値ではありません</h2>
<p>もっとも誤解されやすい点です。
<b>SQ値は225銘柄それぞれの寄り付き価格から算出</b>されます。
一方、日経平均株価の始値は9時00分時点で値が付いている銘柄で計算されるため、
まだ寄り付いていない銘柄があるとその分だけずれます。</p>
""" + _SQ_STATS + """
<p><b>中央値で見れば100円程度</b>なので、たいていの月は近い値になります。
それでも1,000円を超えてずれた月があるということは、
<b>「SQ値 ≒ 始値」と思って建玉を置いておくと、稀に大きく外す</b>ということです。</p>

<h2>過去のSQ値(直近24か月)</h2>
<p>JPXが公表している最終清算数値に、同じ日の日経平均始値を並べました。毎営業日更新しています。</p>
""" + _SQ_TABLE + """
<p>2021年以降の全SQ値と、SQ値にその日の日経平均が届かなかった「幻のSQ」の判定は
<a href="sq-values.html">SQ値一覧</a>にまとめています。</p>
<h2>メジャーSQとマイナーSQ</h2>
<ul>
...
<p>ただし上で見たとおり、<b>始値との乖離という点ではメジャーとマイナーに差はありませんでした。</b>
「メジャーSQだから荒れる」は、少なくともこの指標では確認できません。</p>
...
<p>ただし「SQ前は必ず荒れる」わけではありません。実際には静かに通過する月もあります。</p>

<h2>【実データ】建玉はSQ直前まで「増える」</h2>
<p>「SQが近づくと建玉は整理されて減っていく」と思われがちですが、実際は逆でした。
2026年8月限の建玉推移です。</p>
<ul>
...
むしろ直前になるほど短期の売買が集まります。</p>
<p>建玉が実際に消えるのは<b>SQを通過した瞬間</b>です。
つまり「じわじわ減る」のではなく、<b>直前まで積み上がり、一気に消滅する</b>。
だからこそSQ前後で需給の景色が急に変わります。</p>

<h2>【実データ】期近より期先のほうが建玉が多いことがある</h2>
<p>2026年8月7日(8月SQの1週間前)時点の建玉合計です。</p>
<ul>
<li>8月限(マイナーSQ・残り1週間): <b>169,955枚</b></li>
<li>9月限(メジャーSQ・残り約5週間): <b>189,160枚</b></li>
<li>10月限: 29,792枚</li>
</ul>
<p><b>満期が遠い9月限のほうが約2万枚も多い</b>状態でした。
メジャーSQである9月に、四半期単位のヘッジや長めのポジションが集まるためと考えられます。
<b>期近だけ見ていると市場の関心を見誤る</b>ということです。</p>

<h2>ミニオプションは毎週SQがある</h2>
<p>見落とされがちですが、<b>日経225ミニオプションには週次の満期(ウィークリー)があります</b>。
第2金曜の月次SQに加えて、他の週の金曜にも満期が訪れます。
そのため、月次SQでない週でも<b>満期に伴う需給は毎週発生しています</b>。</p>
<p>ミニは取引単位がラージの1/10で個人が参加しやすく、
短期の値動きへの影響は無視できません。
当サイトではミニの建玉分布も別途掲載しています。</p>

<h2>当サイトでの見方</h2>
<p>トップページに<b>次回SQまでの日数</b>を表示しているほか、
<a href="./#oi">行使価格別の建玉分布</a>を毎営業日更新しています。
SQが近づくにつれて現値の周辺にどれだけ建玉が残っているか、
どの価格帯が厚いままなのかを追うと、満期に向けた地合いが掴みやすくなります。</p>

<h2>あわせて見るもの</h2>
<ul>
<li><a href="guide-oi.html">建玉分布の見方</a> — 「壁」はどう読むか</li>
<li><a href="guide-gex.html">ガンマエクスポージャーとは</a> — SQ通過で需給が反転する仕組み</li>
<li><a href="guide-jpx-data.html">公式データはどこにあるか</a> — SQ値を含む公表ファイルの一覧</li>
...
<h1>先物の手口に出てくる証券会社 — 各社が何をしているのか</h1>
<p>日経225先物の手口を見ていると、毎日同じ会社名が並びます。ＡＢＮクリアリン証券、ソシエテＧ証券、
バークレイズ証券。そして週次の建玉ではＨＳＢＣ証券が常に売り越し、野村証券が常に買い越しです。</p>
<p>「この会社が売っているから下がる」と読みたくなりますが、それはほぼ確実に外れます。
このページでは、直近52週の実測値をもとに<b>各社がどういう性格を持っているのか</b>、
そして<b>手口から何が読めて何が読めないのか</b>を整理します。</p>

<h2>まず、日次の手口と週次の建玉は別物です</h2>
<p>JPXは2種類のデータを出しています。混同すると話が合わなくなります。</p>
<div class="tbl-wrap"><table>
<thead><tr><th></th><th>日次の手口</th><th>週次の建玉</th></tr></thead>
<tbody>
<tr><td>中身</td><td>その日の<b>取引高</b></td><td>週末時点の<b>ネット持ち高</b></td></tr>
<tr><td>方向</td><td><b>分からない</b>(買いと売りの合計)</td><td>分かる(買い越し / 売り越し)</td></tr>
<tr><td>公表</td><td>当日17:45頃</td><td>毎週第1営業日</td></tr>
<tr><td>使い道</td><td>誰が主戦場にいるか</td><td>誰がどちら側にいるか</td></tr>
</tbody>
</table></div>
<p><b>日次の手口に方向はありません。</b>「ＡＢＮクリアリンが6,352枚」は、
買ったのか売ったのかを一切含みません。ここを取り違えている解説をよく見かけます。</p>

<h2>取引高の上位3社は、ほぼ固定です</h2>
<p>2026年9月1日の日経225先物(ラージ)の取引高上位は次のとおりでした。</p>
<div class="tbl-wrap"><table>
<thead><tr><th>参加者</th><th>取引高</th><th>シェア</th></tr></thead>
<tbody>
<tr><td>ＡＢＮクリアリン証券</td><td>6,352枚</td><td>23.7%</td></tr>
<tr><td>ソシエテＧ証券</td><td>6,099枚</td><td>22.7%</td></tr>
<tr><td>バークレイズ証券</td><td>3,700枚</td><td>13.8%</td></tr>
<tr><td>サスケハナ・ホンコン</td><td>1,290枚</td><td>4.8%</td></tr>
</tbody>
</table></div>
<p>この3社で<b>6割</b>を占めます。日によって順番は入れ替わりますが、顔ぶれはほぼ変わりません。</p>
<p>なぜこの3社なのか。いずれも<b>清算・執行を請け負う立場</b>だからです。
海外のヘッジファンドや機関投資家が日本の先物を売買するとき、
自分で取引参加者資格を持たずに、これらの会社を通します。
つまり<b>表示されているのは、その会社自身の相場観ではなく、背後にいる無数の顧客の注文の合計</b>です。</p>
<p>ＡＢＮクリアリン証券は特にこの性格が強く、ミニでは32.6%と3分の1を占めます。
一方で、<b>同社のミニのネット建玉は52週中27週が売り越し・25週が買い越し</b>で、
同符号率は52%。ほぼコイン投げです。方向を読む材料にはなりません。</p>

<h2>ラージでは、8社が52週すべて同じ方向でした</h2>
<p>ここが最も重要な事実です。直近52週(2025年9月〜2026年8月)の日経225先物ラージについて、
各社のネット建玉が毎週どちら側にあったかを数えました。</p>
<div class="tbl-wrap"><table>
<thead><tr><th>参加者</th><th>平均ネット建玉</th><th>同じ符号だった週</th></tr></thead>
<tbody>
<tr><td>ＨＳＢＣ証券</td><td>−33,012枚</td><td><b>52週すべて売り越し</b></td></tr>
<tr><td>野村証券</td><td>+20,759枚</td><td><b>52週すべて買い越し</b></td></tr>
<tr><td>モルガンＭＵＦＧ証券</td><td>−10,759枚</td><td>96%が売り越し</td></tr>
<tr><td>バークレイズ証券</td><td>+7,572枚</td><td>98%が買い越し</td></tr>
<tr><td>ゴールドマン証券</td><td>−6,578枚</td><td>92%が売り越し</td></tr>
<tr><td>ソシエテＧ証券</td><td>+6,574枚</td><td>96%が買い越し</td></tr>
<tr><td>みずほ証券</td><td>−6,237枚</td><td><b>100%が売り越し</b></td></tr>
<tr><td>三菱ＵＦＪ証券</td><td>−5,600枚</td><td><b>52週すべて売り越し</b></td></tr>
<tr><td>ＵＢＳ証券</td><td>+4,901枚</td><td><b>52週すべて買い越し</b></td></tr>
<tr><td>シティグループ証券</td><td>+4,888枚</td><td><b>52週すべて買い越し</b></td></tr>
<tr><td>ＳＭＢＣ日興証券</td><td>+3,972枚</td><td><b>52週すべて買い越し</b></td></tr>
<tr><td>ドイツ証券</td><td>+2,125枚</td><td><b>52週すべて買い越し</b></td></tr>
</tbody>
</table></div>
<p><b>この1年、日経平均は大きく上下しています。</b>それでも上の会社は一度も向きを変えていません。
相場観でポジションを取っているなら、こうはなりません。</p>
<p>理由は業務の形にあります。仕組債やデリバティブを顧客に売った側は、
そのリスクを打ち消すために反対のポジションを持ち続けます。
現物株を大量に保有していれば、先物でヘッジを掛けたままになります。
裁定取引をしていれば、現物と先物で常に反対側を持ちます。
<b>いずれも相場観とは無関係に、恒常的に同じ方向のポジションを生みます。</b></p>
<p>したがって<b>「ＨＳＢＣが売り越しだから弱気」と読むと、1年間ずっと外し続けます。</b>
見るべきはゼロとの比較ではなく、<b>その会社自身の平常値からのズレ</b>です。
平常が−33,000枚の会社が−5,000枚になっていれば、符号は売り越しのままでも、
実質は28,000枚を買い戻したことになります。</p>

<h2>ミニは性格がまったく違います</h2>
<p>同じ52週を、日経225miniで集計すると景色が変わります。</p>
<div class="tbl-wrap"><table>
<thead><tr><th>参加者</th><th>平均ネット建玉</th><th>同符号率</th></tr></thead>
<tbody>
<tr><td>バークレイズ証券</td><td>−11,806枚</td><td>73%</td></tr>
<tr><td>ＢＮＰパリバ証券</td><td>−8,852枚</td><td>73%</td></tr>
<tr><td>みずほ証券</td><td>+6,685枚</td><td>62%</td></tr>
<tr><td>ソシエテＧ証券</td><td>+4,602枚</td><td>60%</td></tr>
<tr><td>ＪＰモルガン証券</td><td>+2,580枚</td><td><b>50%</b></td></tr>
<tr><td>ＡＢＮクリアリン証券</td><td>+1,009枚</td><td>52%</td></tr>
</tbody>
</table></div>
<p>ラージでは100%だった会社が、ミニでは50〜73%です。<b>ミニのポジションは動きます。</b></p>
<p>さらに面白いのは、<b>同じ会社がラージとミニで逆を向いている</b>ケースがあることです。</p>
<ul>
<li><b>バークレイズ証券</b> — ラージは+7,572枚(98%買い越し)、ミニは−11,806枚(73%売り越し)</li>
<li><b>みずほ証券</b> — ラージは−6,237枚(100%売り越し)、ミニは+6,685枚(62%買い越し)</li>
<li><b>ゴールドマン証券</b> — ラージは−6,578枚(92%売り越し)、ミニは+3,732枚(63%買い越し)</li>
</ul>
<p>ラージとミニを合算して「この会社は買い越し」と語ると、この構造が消えます。
当サイトが商品別に分けて表示しているのはこのためです。</p>

<h2>ミニにしか出てこない会社があります</h2>
<p>ミニの上位には、ラージには現れない名前が並びます。日産証券、ＳＢＩ証券、松井証券、楽天証券、
三菱ＵＦＪ eスマート証券、インタラクティブ証券。<b>いずれも個人投資家向けのブローカーです。</b></p>
<p>9月1日のミニ取引高でも、ＳＢＩ証券5.9%、松井証券4.1%、楽天証券3.7%が上位に入っています。
ラージの同じ日の上位10社には、これらの名前はありません。</p>
<p>つまり<b>ラージは機関投資家、ミニは個人の比重が高い</b>という住み分けが、手口にそのまま出ています。
Put/Callレシオがラージ平均1.57・ミニ平均0.91と大きく違うのも、同じ理由です
(<a href="guide-pcr.html">→ Put/Callレシオとは</a>)。</p>

<h2>結局、手口から何が読めるのか</h2>
<p><b>読めること</b></p>
<ul>
<li><b>その会社自身の平常値からのズレ</b> — 平常−33,000枚が−5,000枚なら、実質的な買い戻し</li>
<li><b>ラージとミニの温度差</b> — 機関と個人が違う方向を向いているかどうか</li>
<li><b>誰が主戦場にいるか</b> — 取引高の顔ぶれが変われば、参加者の構成が変わったサイン</li>
</ul>
<p><b>読めないこと</b></p>
<ul>
<li><b>日次の手口から売買の方向</b> — データに含まれていません</li>
<li><b>自己勘定か顧客注文か</b> — 分離されていません。清算会社の数字はほぼ顧客のものです</li>
<li><b>その会社の相場観</b> — 上の表のとおり、ポジションは業務構造で決まっています</li>
</ul>
<p>手口は<b>フローの指紋であって、予想ではありません。</b>
そう割り切ったうえで平常値との差を見ると、はじめて使える材料になります。</p>

<p>当サイトでは、<a href="./">日経ダッシュボード</a>に日次の取引高上位と週次の参加者別建玉を、
<a href="tools.html">データ分析ツール</a>に会社ごとの時系列を掲載しています。いずれも毎営業日更新です。
基本的な読み方は <a href="guide-teguchi.html">先物の手口の見方</a> にまとめています。</p>
"""),
...
"""),
    "guide-oi.html": ("日経225オプションの建玉分布(毎営業日更新)｜今日の壁と見方", """
<h1>日経225オプションの建玉分布 — 今日の「壁」と、その見方</h1>
{latest_oi}
...
<tr><td>マネックス証券</td><td><b>取扱いあり</b><br>0.198%(最低19.8円)</td><td>0.198%<br>(最低198円)</td><td>275円</td><td>38.5円</td><td>11円</td></tr>
</tbody>
</table></div>
<p style="font-size:.9em;color:#666">手数料は1枚あたり・税込。2026年8月22日(マネックス証券は9月13日)に各社公式サイトで確認した値です。
...
<li><b>ラージ先物中心なら松井証券が安い</b> — 220円で4社中最安です。
松井証券には返済期限をその日のセッション内に限る「一日先物取引」もあり、
日中で完結させる使い方ならさらに低コストになります</li>
<li><b>オプションの手数料率は、この表の4社で2通り</b> — 0.22%の3社と、0.198%のマネックス証券です。
日経225オプション(ラージ)の最低手数料は、前者が220円、後者が198円です</li>
<li><b>マイクロ先物は4社とも11円で横並び</b> — 日経225miniのさらに10分の1の単位で、
...
ただし<b>ラージ先物が220円と4社で最安</b>で、返済期限をその日のセッション内に限る
...
<h3>マネックス証券</h3>
<p>ミニオプションを扱う3社目です。手数料は売買代金の0.198%(最低19.8円)で、
SBI証券の0.22%と同じ「率」の体系ですが、率そのものが1割低くなっています。
最低手数料は同じ19.8円なので、遠い行使価格を数百円で買う使い方では差が出ません。
差が出るのはプレミアムが高いときです。</p>
<p>三菱UFJ eスマート証券の定額19.8円と比べると、分かれ目は売買代金1万円(ミニで100ポイント)です。
それより安いオプションなら同額、高いオプションなら定額のほうが安くなります。
たとえばミニで150ポイント(1.5万円)なら、マネックス証券は29.7円、三菱UFJ eスマート証券は19.8円です。</p>
<p>日経225オプション(ラージ)では、この表の4社の中で手数料が低い水準です。
売買代金10万円まではどの社も最低手数料の範囲ですが、そこでマネックス証券は198円、他の3社は220円です。
先物は275円・38.5円・11円で、SBI証券・三菱UFJ eスマート証券と同じです。
口座開設は、証券総合取引口座を開いたうえで先物・オプション取引口座を申し込む流れで、どちらも無料です。</p>
<p>(PR) <a href="https://h.accesstrade.net/sp/cc?rk=01009fms00ovxn" rel="nofollow" referrerpolicy="no-referrer-when-downgrade">マネックス証券<img src="https://h.accesstrade.net/sp/rr?rk=01009fms00ovxn" width="1" height="1" border="0" alt=""></a>(公式)</p>
...
<!-- 提携状況(2026-09-07にアクセストレード管理画面で再確認)。
     承認されたものからここに広告リンクを差し替える。

     設置済み(アクセストレード):
       三菱UFJ eスマート証券(952022) 定額報酬3,000円 ← 2026-09-07 承認・設置
       マネックス証券(349213) 口座開設申込3,000円 ← 2026-09-26 承認・設置
         広告素材(440164)「マネックス証券」のテキストリンクを、マネックス証券の解説の直後に設置。
         リンクコードは改変禁止(第15条)のためアンカー文言は原文のまま。PR表記はアンカーの外。
         掲載面の文章は、最上級表現と強調を外す修正済み(金融アフィリエイトのガイドライン対応)。
       SBI証券(38299) 資料請求300円
     設置済み(A8.net): 松井証券 / DMM株

     ※リンクコードは改変禁止(パートナー利用規約15条)。アンカーテキストも
       素材のまま貼ること。説明を足す場合は </a> の外に書く。
       SBI証券の「SBI証券の日経225」は素材そのままで、改変ではない(2026-09-07確認)。
...
if _SQ_CAL:
    ...
    # 「10月sq」のように月名で探す人がいる(9月時点で25位)。月名を先頭に入れる。
    _title = f"SQはいつ？{_sq.month}月SQは{_sq.day}日({_W[_sq.weekday()]})｜{_sq.year}年SQ日程一覧"
    ...
    _new_h1 = (f"<h1>SQはいつ？ {_sq.month}月SQは{_sq.month}月{_sq.day}日({_W[_sq.weekday()]})"
