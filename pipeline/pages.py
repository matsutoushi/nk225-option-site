...
<!-- 提携状況(2026-08-21に各ASPの管理画面で実際に確認した内容)。
     ...
     アクセストレード 提携却下 → 再申請の交渉中:
       SBI証券 300円 / イデコSBI証券 300円 / 三菱UFJ eスマート証券 3,000円
       却下分は管理画面から再申請できない(お問い合わせ区分[提携>再申請]が必要)。
       2026-08-21に再申請を依頼し、2026-08-22にASPから回答。
       いずれも「広告主による掲載面チェックあり・当該サービスに関する記事提出が必須」。
       → SBI証券 / 三菱UFJ eスマート証券:
         上の「取引口座を用意する」に3社の手数料比較表を追加した(2026-08-22)。
         この節を掲載面として提出する。承認後に両社のリンクをここへ入れる。
       → イデコSBI証券: 見送る。iDeCoでは先物・オプションを扱えず、
         このページの読者の目的と重ならない。審査を通すためだけの記事は書かない。
       SBI証券本体はA8に出稿が無いため、この経路以外に手段が無い。
       三菱UFJ eスマート証券はA8に未提携で存在する(証券口座3,000円 / FX 7,000円)ので、
       アクセストレードの回答が却下だった場合はA8から申請する。

     楽天証券(TGアフィリエイト)の申請状況は未確認。 -->
<!-- 【アフィリエイトリンク: SBI証券 口座開設】 -->
